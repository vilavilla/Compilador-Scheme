...
class EvalVisitor(SchemeVisitor):
    ...
    def visitLogicalExpr(self, ctx):
        operator = ctx.getChild(1).getText()
        args = [bool(self.visit(expr)) for expr in ctx.expr()]
        print(f"DEBUG: visitLogicalExpr - Operator: {operator}, Args: {args}")
        result = args[0]
        for arg in args[1:]:
            result = LogicDicc[operator](result, arg)
        print(f"DEBUG: visitLogicalExpr - Result: {result}")
        return result
    ...
    def validate_identifier(self, identifier):
        import re
        # Regex para solo letras, números y subrayados, comenzando con una letra
        if not re.match(r"^[a-zA-Z][a-zA-Z0-9_-]*$", identifier):
            raise Exception(f"Error: Identificador inválido '{identifier}' (no se permiten caracteres como '-', '.', etc.).")

    def visitFunctionCallExpr(self, ctx):
        func_name = ctx.ID().getText()  # Nombre de la función
        self.validate_identifier(func_name)  # Validar identificador

        if func_name not in FunctionTable:
            raise Exception(f"Error: Función '{func_name}' no definida.")

        # Recuperar parámetros, bloque y ámbito local
        params, block, local_scope = FunctionTable[func_name]

        # Evaluar argumentos
        args = [self.visit(arg) for arg in ctx.expr()]
        if len(args) != len(params):
            raise Exception(f"Error: Número incorrecto de argumentos para la función '{func_name}'.")

        # Crear un nuevo scope con los argumentos
        scope = dict(zip(params, args))
        SymbolTable.append(scope)

        # Añadir funciones anidadas al scope
        if local_scope:
            SymbolTable[-1].update(local_scope)
    ...
    def visitFunctionDeclaration(self, ctx):
        """
        Maneja la definición de funciones, incluyendo funciones anidadas.
        """
        function_name = ctx.ID(0).getText()  # Nombre de la función principal
        params = [param.getText() for param in ctx.ID()[1:]]  # Parámetros
        block = ctx.block()  # Bloque de la función

        # Guarda la función principal en la tabla global
        FunctionTable[function_name] = (params, block)

        # Si hay funciones anidadas, manejarlas como variables locales
        local_scope = {}
        for stmt in block.stmt():
            if isinstance(stmt, SchemeParser.FunctionDeclarationContext):
                nested_function_name = stmt.ID(0).getText()
                nested_params = [param.getText() for param in stmt.ID()[1:]]
                nested_block = stmt.block()
                local_scope[nested_function_name] = (nested_params, nested_block)

        SymbolTable[-1].update(local_scope)  # Añadir funciones anidadas al ámbito actual
    ...
    def visitDeclaration(self, ctx):
        ...
        # Delegar según el tipo de declaración
        ...
        print(f"DEBUG: visitDeclaration - Resultado: {result}")
        return result
    ...
    def visitCdrExpr(self, ctx):
        ...
        result = lst[1:] if len(lst) > 0 else []
